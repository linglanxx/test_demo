...
{      	
	  unsigned char c=0,i=0,tmp,j=0;	
		c=chr-' ';//得到偏移后的值 也就是索引号，在表中第几个位置			
		if(x>WIDTH-1){x=0;y=y+2;} //行数超过范围
		/* 16x8，高为16，宽8，即有16行8列，先取其中一行，然后依次显示这一行的8列，如此循环16行，完成显示一个字符 */
        if(Char_Size ==16)
		{
			for(i=0;i<16;i++)
			{
			  if(mode)
				{
					tmp = F8X16[c*16+i];  // 把将显示值的16行像素点给tmp
				}
				else
				{
					tmp = ~(F8X16[c*16+i]);
				...
				for(j=0;j<8;j++)
				{
					if(tmp&(0x80>>j))
					{
						OLED_Set_Pixel(x+j, y+i,1); //显示当前行每一列的像素点
					}
					else
					{
 ...
 * @parameters :x:the bebinning x coordinate of the English string
                y:the bebinning y coordinate of the English string
								...
								Char_Size:the size of display character (字体大小：8、16、32)
								mode:0-white background and black character
								     1-black background and white character
 * @retvalue   :None
******************************************************************************/   	  
 ...
 * @function   :Display a single 16x16 Chinese character
                (根据index[]索引需要显示的字在字库中的位置，再将各个像素点进行显示)
